  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        ...
        // backend may return a string message when there are no submissions
        // normalize to an array so the UI behaves consistently
        ...
        // show backend/auth errors when available
        if (err?.response?.status === 401) {
          setError('You must be logged in to view submissions.');
        } else if (err?.response?.data) {
          setError(typeof err.response.data === 'string' ? err.response.data : 'Failed to fetch submission history');
        } else {
